...
{
	int vertCount = row * cols;
	Vertex* aoVertices = new Vertex[vertCount];
	for (unsigned int r = 0; r < row; r++)
	{
		for (unsigned int c = 0; c < cols; c++)
		{
			...
			aoVertices[r*cols + c].uv = glm::vec2(rowNorm, colNorm);
		}
		
	}
	//index count
	int indexCount = (row - 1) * (cols - 1) * 6;
	...
	glBufferData(GL_ARRAY_BUFFER, vertCount*sizeof(Vertex), aoVertices, GL_STATIC_DRAW);
